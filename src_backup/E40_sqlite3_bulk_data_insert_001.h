...
void E40_run() {
	...
	// Open database 1
	if (E40_db_open("/sdcard/bulk_ins.db", &db1))
		return;

	displayFreeHeap();

	rc = E40_db_exec(db1, "CREATE TABLE IF NOT EXISTS test (c1 INTEGER, c2, c3, c4, c5 INTEGER, c6 INTEGER, c7, c8, c9 DATETIME, c10 DATETIME, c11 INTEGER )");
	if (rc != SQLITE_OK) {
		sqlite3_close(db1);
		return;
	}
	...
	rc = E40_db_exec(db1, "Select count(*) from test");
	if (rc != SQLITE_OK) {
		sqlite3_close(db1);
		return;
	}
